...
export default function SimulationHistoryPage(): JSX.Element {
  ...
  const [selectedInterview, setSelectedInterview] =
    useState<SelectedInterview | null>(null);
  const [expandedSimulations, setExpandedSimulations] = useState<Set<string>>(
    new Set()
  );
  ...
  const renderDemographicInfo = (demographics: any) => {
    ...
    const validItems = demographicItems.filter(
      (item) => demographics[item.key]
    );
  ...
  const fetchSimulationHistory = useCallback(async () => {
    ...
    try {
      ...
      const localStorageData = JSON.parse(
        localStorage.getItem("simulation_results") || "[]"
      );
      ...
      try {
        ...
        console.warn(
          "Backend API unavailable, using localStorage only:",
          backendError
        );
      }
      ...
      localStorageData.forEach((localSim: any, index: number) => {
        ...
        if (simId) {
          ...
          console.warn(
            `⚠️ No simulation ID found in localStorage item ${index}:`,
            localSim
          );
        }
      });
      ...
      console.log(
        ...
        Array.from(allSimulations.keys())
      );
        ...
        Array.from(allSimulations.values()).map(async ({ source, data }) => {
          try {
            ...
            if (source === "localStorage" && simulationData.interviews) {
              ...
              businessContext =
                simulationData.business_context ||
                simulationData.metadata?.business_context ||
                "";
              ...
              simulationData.interviews?.forEach((interview: any) => {
                ...
                const persona =
                  simulationData.people?.find(
                    (p: any) =>
                      p.id === interview.persona_id ||
                      p.id === interview.person_id
                  ) ||
                  simulationData.data?.personas?.find(
                    (p: any) =>
                      p.id === interview.persona_id ||
                      p.id === interview.person_id
                  );
                ...
                console.log(
                  `🔍 Processing localStorage interview for ${stakeholderType}:`,
                  {
                    ...
                    available_people: [
                      ...(simulationData.people?.map((p: any) => ({
                        id: p.id,
                        name: p.name,
                      })) || []),
                      ...(simulationData.data?.personas?.map((p: any) => ({
                        id: p.id,
                        name: p.name,
                      })) || []),
                    ],
                ...
                const interviewData: InterviewData = {
                  id:
                    interview.id ||
                    `${interview.persona_id || interview.person_id}_${Date.now()}`,
                  personaName:
                    persona?.name || `${stakeholderType} Representative`,
                  personaDetails: {
                    name: persona?.name || `${stakeholderType} Representative`,
                    background: persona?.background || persona?.bio || "",
                    role:
                      persona?.role || persona?.job_title || stakeholderType,
                    experience:
                      persona?.experience || persona?.years_experience || "",
                    age: persona?.age,
                    demographic_details:
                      persona?.demographic_details || persona?.demographics,
                  },
              ...
              allEntries.forEach(({ interview }) => {
                const base = (interview.personaDetails.name || "")
                  .split(",")[0]
                  .trim();
                if (!base) return;
                baseNameCounts.set(base, (baseNameCounts.get(base) || 0) + 1);
              });
              const duplicates = Array.from(baseNameCounts.entries())
                .filter(([, c]) => c > 1)
                .map(([n]) => n);
              if (duplicates.length > 0) {
                const dupInfo = duplicates.map((base) => {
                  const samples = allEntries
                    .filter(
                      (e) =>
                        (e.interview.personaDetails.name || "")
                          .split(",")[0]
                          .trim() === base
                    )
                    .map((e) => ({
                      stakeholderType: e.stakeholderType,
                      role: e.interview.personaDetails.role,
                      age: e.interview.personaDetails.age,
                    }));
                  return { base, samples };
                });
                console.warn(
                  "🔁 Duplicate person base names across clusters (localStorage):",
                  dupInfo
                );
              }
              ...
              stakeholders = Array.from(stakeholderMap.entries()).map(
                ([type, interviews]) => ({
                  type,
                  interviews,
                })
              ...
              try {
                ...
                let detailResponse = await fetch(
                  `${API_BASE_URL}/api/research/simulation-bridge/completed-item?simulation_id=${encodeURIComponent(simulationId)}`,
                  {
                    headers: {
                ...
                if (!detailResponse.ok) {
                  detailResponse = await fetch(
                    `/api/research/simulation-bridge/completed/${simulationId}`
                  );
                }
                if (detailResponse.ok) {
                  const detailData = await detailResponse.json();
                  businessContext =
                    detailData.business_context ||
                    detailData.metadata?.business_context ||
                    "";

                  // Group interviews by stakeholder type
                  const stakeholderMap = new Map<string, InterviewData[]>();
                  const allEntries2: Array<{
                    interview: InterviewData;
                    stakeholderType: string;
                  }> = [];
                  ...
                  detailData.interviews?.forEach((interview: any) => {
                    ...
                    // Check both .people and .data.personas for persona data
                    const persona =
                      detailData.people?.find(
                        (p: any) =>
                          p.id === interview.persona_id ||
                          p.id === interview.person_id
                      ) ||
                      detailData.data?.personas?.find(
                        (p: any) =>
                          p.id === interview.persona_id ||
                          p.id === interview.person_id
                      );
                    ...
                    console.log(
                      `🔍 Processing backend interview for ${stakeholderType}:`,
                      {
                        interview_id: interview.id,
                        persona_id: interview.persona_id,
                        person_id: interview.person_id,
                        ...
                        persona_data: persona,
                        available_people: [
                          ...(detailData.people?.map((p: any) => ({
                            id: p.id,
                            name: p.name,
                          })) || []),
                          ...(detailData.data?.personas?.map((p: any) => ({
                            id: p.id,
                            name: p.name,
                          })) || []),
                        ],
                  ...
                  allEntries2.forEach(({ interview }) => {
                    ...
                    baseNameCounts2.set(
                      base,
                      (baseNameCounts2.get(base) || 0) + 1
                    );
                  });
                  const duplicates2 = Array.from(baseNameCounts2.entries())
                    .filter(([, c]) => c > 1)
                    .map(([n]) => n);
                  if (duplicates2.length > 0) {
                    ...
                    console.warn(
                      "🔁 Duplicate person base names across clusters (backend):",
                      dupInfo2
                    );
                  }
                ...
                console.warn(
                  `Failed to fetch backend details for ${simulationId}:`,
                  detailError
                );
              }
            }
            ...
            const metaCreatedAt =
              simulationData?.metadata?.created_at ||
              simulationData?.data?.metadata?.created_at;
            const createdAt =
              metaCreatedAt ||
              (source === "localStorage" ? data.timestamp : data.created_at) ||
              new Date().toISOString();

            // Prefer totals from detailed data (simulationData.metadata) when available; fallback to summary
            const totalPersonas =
              simulationData?.metadata?.total_personas ??
              data.total_personas ??
              0;
            const totalInterviews =
              simulationData?.metadata?.total_interviews ??
      ...
      setError(
        err instanceof Error
          ? err
          : new Error("Failed to load simulation history")
      );
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === "simulation_results") {
        console.log(
          "🔄 localStorage simulation_results changed, refreshing..."
        );
        fetchSimulationHistory();
      }
    };
  ...
  const handleAnalyze = async (simulationId: string) => {
    try {
      ...
      const response = await fetch(
        `/api/research/simulation-bridge/analyze/${simulationId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (result.success) {
        ...
        console.log(
          ...
          result.next_steps.analysis_url_pattern
        );
      } else {
  ...
  const handleDownload = async (simulationId: string) => {
    try {
      ...
      const localSim = localStorageData.find(
        (sim: any) =>
          sim.simulation_id === simulationId ||
          sim.results?.simulation_id === simulationId
      );
      ...
      const businessContext =
        result.business_context || result.metadata?.business_context;
      if (businessContext) {
      ...
      // Stakeholder summary
      const stakeholderGroups = result.interviews.reduce(
        (acc: any, interview: any) => {
      ...
      content += `${"-".repeat(21)}\n`;
      Object.entries(stakeholderGroups).forEach(
        ([type, interviewList]: [string, any]) => {
      ...
      result.interviews.forEach((interview: any, index: number) => {
        const person =
          result.people?.find(
            (p: any) => p.id === (interview.person_id || interview.persona_id)
          ) ||
          result.personas?.find(
            (p: any) => p.id === (interview.person_id || interview.persona_id)
          );
        const baseTimestamp = new Date();
        const interviewStartTime = new Date(
          baseTimestamp.getTime() + index * 30 * 60 * 1000
        ); // 30 minutes between interviews
        ...
        if (person) {
          ...
          if (person.profile?.motivations || person.profile?.pain_points) {
            ...
            if (person.profile.motivations) {
              ...
              person.profile.motivations.forEach(
                (motivation: string, i: number) => {
        ...
        if (interview.responses && interview.responses.length > 0) {
          interview.responses.forEach((response: any, i: number) => {
            const questionTime = new Date(
              interviewStartTime.getTime() + i * 3 * 60 * 1000
            ); // 3 minutes per Q&A
            const responseTime = new Date(
              questionTime.getTime() + 1 * 60 * 1000
            ); // 1 minute later
      ...
      showToast("Enhanced interviews downloaded successfully", {
        variant: "success",
      });
    } catch (error) {
      ...
      <div className="flex h-[calc(100vh-8rem)] w-full">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center space-y-4">
            ...
            <Button variant="outline" size="sm" onClick={debugLocalStorage}>
              Debug localStorage
            </Button>
          </div>
        </div>
        ...
        {selectedInterview ? (
          <>
              ...
              <div className="space-y-2">
                ...
                {selectedInterview.businessContext && (
                  <details className="group">
                    <summary className="cursor-pointer flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-950/20 rounded border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-950/30">
                      ...
                      <span className="font-medium text-blue-900 dark:text-blue-100 text-xs">
                        Business Context
                      </span>
                      <span className="ml-auto text-xs text-blue-600 dark:text-blue-400 group-open:rotate-90 transition-transform">
                        ▶
                      </span>
                    </summary>
                    <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-950/20 rounded border border-blue-200 dark:border-blue-800">
                      <div className="text-xs text-blue-800 dark:text-blue-200 space-y-1">
                        {(() => {
                          try {
                            if (
                              typeof selectedInterview.businessContext ===
                              "object"
                            ) {
                            ...
                            return (
                              <p>{String(selectedInterview.businessContext)}</p>
                            );
                          }
                        })()}
                      </div>
                ...
                <details className="group">
                  <summary className="cursor-pointer flex items-center gap-2 p-2 bg-green-50 dark:bg-green-950/20 rounded border border-green-200 dark:border-green-800 hover:bg-green-100 dark:hover:bg-green-950/30">
                    ...
                    <span className="ml-auto text-xs text-green-600 dark:text-green-400 group-open:rotate-90 transition-transform">
                      ▶
                    </span>
                  </summary>
                  <div className="mt-2 p-3 bg-green-50 dark:bg-green-950/20 rounded border border-green-200 dark:border-green-800">
                    <div className="space-y-2">
                      ...
                      {renderDemographicInfo(
                        selectedInterview.interview.personaDetails
                          .demographic_details
                      )}
                      ...
                      {selectedInterview.interview.personaDetails
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs mt-2">
                          <div>
                            ...
                            <p>
                              {selectedInterview.interview.personaDetails.role}
                            </p>
                          </div>
            ...
            <ScrollArea className="flex-1 min-h-0">
              <div className="p-4 space-y-4 max-w-4xl">
                {selectedInterview.interview.responses.map(
                  (response, index) => (
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-muted-foreground text-sm">
                Choose a simulation and persona from the left to view the
                interview details
              </p>
            </div>
          </div>
