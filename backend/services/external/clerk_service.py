...
class ClerkService:
    ...
    def __init__(self):
        ...
        # JWKS URL must be supplied via env (e.g. CLERK_JWKS_URL=https://<your-tenant>.clerk.accounts.dev/.well-known/jwks.json)
        self.jwks_url = os.getenv("CLERK_JWKS_URL", "")
        self.clerk_api_url = os.getenv("CLERK_API_URL", "https://api.clerk.com")
    ...
    def _load_jwks(self) -> None:
        ...
        if not self.jwks_url:
            logger.warning("CLERK_JWKS_URL is not set; skipping JWKS load.")
            return
        try:
    ...
    def validate_token(self, token: str) -> Tuple[bool, Optional[Dict[Any, Any]]]:
        ...
        try:
            ...
            # CRITICAL FIX: Return the payload that was extracted successfully
            return True, payload
