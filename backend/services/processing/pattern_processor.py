...
class PatternProcessor(IProcessor):
    ...
    async def process(self, data: Any, context: Optional[Dict[str, Any]] = None) -> PatternResponse:
        """
        ...
        try:
            ...
            if "themes" in context:
                ...
                patterns = await self._generate_patterns_from_themes(context["themes"], text)
                return PatternResponse(patterns=patterns)
            
            # Return empty patterns if all else fails
            logger.warning("All pattern generation methods failed, returning empty patterns")
            return PatternResponse(patterns=[])
    ...
    async def _generate_patterns_from_themes(
        ...
        for theme in themes:
            ...
            if not theme.get("name") or not (theme.get("definition") or theme.get("description")):
                continue
            
            # Extract theme data
            name = theme.get("name", "Unknown Theme")
            description = theme.get("definition") or theme.get("description", "No description available.")
