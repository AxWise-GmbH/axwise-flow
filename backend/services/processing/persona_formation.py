...
class PersonaFormationService:
                ...
                try:
                    ...
                    if attributes and isinstance(attributes, dict) and attributes.get('confidence', 0) >= self.validation_threshold:
                        try:
                            ...
                            key_responsibilities_data = attributes.get('key_responsibilities', {}) # Corrected key name
                            ...
                            persona = Persona(
                                ...
                                confidence=float(attributes.get('confidence', 0.7)), # Use default if missing and ensure float
    ...
    async def _analyze_patterns_for_persona(self, patterns: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        prompt = self._get_direct_persona_prompt_nested(pattern_descriptions) # Reusing direct text prompt structure
    ...
    def _create_fallback_persona_attributes(self, patterns=None):
         ...
         return {
                ...
                "role_context": {"value": "", "confidence": 0.5, "evidence": []},
                "key_responsibilities": {"value": "", "confidence": 0.5, "evidence": []},
                "tools_used": {"value": "", "confidence": 0.5, "evidence": []},
                "collaboration_style": {"value": "", "confidence": 0.5, "evidence": []},
                "analysis_approach": {"value": "", "confidence": 0.5, "evidence": []},
                "pain_points": {"value": "", "confidence": 0.5, "evidence": []},
    ...
    async def _create_default_persona(self, context: Optional[Dict[str, Any]] = None) -> List[Persona]:
        ...
        try:
            ...
            prompt = self._get_direct_persona_prompt_nested(original_text) # Use new prompt helper
            ...
            if persona_data and isinstance(persona_data, dict): # Check if we have valid data
                ...
                try:
                    ...
                    key_responsibilities_data = persona_data.get('key_responsibilities', {})
                    tools_used_data = persona_data.get('tools_used', {})
                    collaboration_style_data = persona_data.get('collaboration_style', {})
                    ...
                    persona = Persona(
                        ...
                        evidence=self._clean_evidence_list(persona_data.get('evidence', [])),
                        persona_metadata=self._get_text_metadata(original_text, context)
                    )
    ...
    def _get_direct_persona_prompt(self, text: str) -> str:
        ...
        logger.warning("_get_direct_persona_prompt is deprecated, use _get_direct_persona_prompt_nested")
    ...
    async def generate_persona_from_text(self, text: str, context: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            prompt = self._get_direct_persona_prompt_nested(text) # Use nested prompt
            ...
            if attributes is None and hasattr(self.llm_service, '_make_request'):
                try:
                    ...
                    if attributes:
                         logger.info("Successfully used _make_request method as fallback")
                except Exception as e:
            ...
            if attributes and isinstance(attributes, dict):
                try:
                    ...
                    persona = Persona(
                        ...
                        confidence=float(attributes.get('confidence', 0.7)), # Use overall confidence from LLM
