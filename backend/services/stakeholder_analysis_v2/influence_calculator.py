...
"""
        ...
        for concern_group in concern_groups:
            if len(concern_group) >= 2:  # At least 2 stakeholders share this concern
                consensus_area = ConsensusArea(
                    ...
                    participating_stakeholders=[s.stakeholder_id for s in detected_stakeholders[:len(concern_group)]],
                    consensus_strength=min(len(concern_group) / len(detected_stakeholders), 1.0),
    ...
    def _heuristic_analyze_conflicts(
        ...
        if len(unique_types) >= 2:
            ...
            conflict_zone = ConflictZone(
                ...
                resolution_suggestions=["Facilitate cross-stakeholder communication", "Identify common ground"],
            )
    ...
    def _calculate_pattern_metrics(
        ...
        total_patterns = len(consensus_areas) + len(conflict_zones) + len(influence_networks)
        ...
        overall_strength = (consensus_strength + conflict_strength + influence_strength) / max(total_patterns, 1)
    ...
    def _create_minimal_patterns(
        ...
        return CrossStakeholderPatterns(
            ...
            metadata={"fallback": True, "stakeholder_count": len(detected_stakeholders)},
        )
