...
def analyze_persona_keywords(persona: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for trait in trait_fields:
        ...
        if all_keywords:
            ...
            analysis["traits"][trait] = {
                ...
                "sample_evidence": (
                    evidence[:2] if evidence else []
                ),  # First 2 quotes for examples
            }

            # Update overall stats
            analysis["overall_stats"]["total_keywords"] += keyword_analysis[
                "total_keywords"
            ]
            analysis["overall_stats"]["generic_count"] += keyword_analysis[
                "generic_count"
            ]
            analysis["overall_stats"]["domain_specific_count"] += keyword_analysis[
    ...
    if total > 0:
        ...
        if generic_pct > 40:
            analysis["overall_stats"]["quality_issues"].append(
                f"Too many generic keywords ({generic_pct:.1f}%)"
            )
        if domain_pct < 20:
            analysis["overall_stats"]["quality_issues"].append(
                f"Insufficient domain-specific keywords ({domain_pct:.1f}%)"
            )
        if total < 10:
            analysis["overall_stats"]["quality_issues"].append(
                f"Too few keywords highlighted ({total} total)"
            )
...
def main():
    """Analyze keyword highlighting quality in the most recent analysis."""

    result_id = 176  # Analysis with enhanced validation system
    ...
    try:
        ...
        if not analysis:
            print(f"❌ Analysis result {result_id} not found")
            return
        ...
        if "personas" not in results_data:
        ...
        for i, persona in enumerate(personas):
            ...
            print(
                f"   • Generic keywords: {stats['generic_count']} ({stats.get('generic_percentage', 0):.1f}%)"
            )
            print(
                f"   • Domain-specific keywords: {stats['domain_specific_count']} ({stats.get('domain_specific_percentage', 0):.1f}%)"
            )
            ...
            for trait, trait_analysis in persona_analysis["traits"].items():
                ...
                if kw_analysis["generic_keywords"]:
                    print(
                        f"      Generic keywords: {', '.join(kw_analysis['generic_keywords'][:5])}"
                    )
                if kw_analysis["domain_specific_keywords"]:
                    print(
                        f"      Domain-specific: {', '.join(kw_analysis['domain_specific_keywords'][:5])}"
                    )
                ...
                if trait_analysis["sample_evidence"]:
                    ...
                    for j, quote in enumerate(
                        trait_analysis["sample_evidence"][:1]
                    ):  # Show 1 example
                        highlighted_kws = extract_highlighted_keywords(quote)
                        print(f'         "{quote[:100]}..."')
                        if highlighted_kws:
                            print(
                                f"         Highlighted: {', '.join(highlighted_kws[:3])}"
                            )
        ...
        personas_with_issues = sum(
            1
            for p in personas
            if analyze_persona_keywords(p)["overall_stats"]["quality_issues"]
        )
        ...
        print(
            f"   • Success rate: {((total_personas - personas_with_issues) / total_personas * 100):.1f}%"
        )

        print(f"\n💡 Recommendations:")
        print(f"   1. Reduce generic keyword highlighting (like, they, process)")
        print(
            f"   2. Increase domain-specific keywords (Apple, price, device, discrimination)"
        )
        print(f"   3. Improve keyword selection algorithm to focus on meaningful terms")
        print(f"   4. Consider context-aware highlighting based on interview domain")
