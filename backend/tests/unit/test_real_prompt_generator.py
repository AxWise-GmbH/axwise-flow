#!/usr/bin/env python3
"""
Test the real prompt generator to see what prompts it creates vs our high-quality test prompt.
"""
...
async def test_real_prompt_generator():
    """Test what the real prompt generator creates."""

    print("🔍 TESTING REAL PROMPT GENERATOR")
    print("=" * 80)
    ...
    try:
        ...
        llm_service = GeminiService(config)
        persona_service = PersonaFormationService(config, llm_service)

        print("✅ PersonaFormationService created")
        ...
        if (
            hasattr(persona_service, "prompt_generator")
            and persona_service.prompt_generator
        ):
            print("✅ Prompt generator found")

            # Generate the real prompt
            real_prompt = (
                persona_service.prompt_generator.create_simplified_persona_prompt(
...
OUTPUT: Complete SimplifiedPersona with all fields populated with rich, specific content."""
            ...
            real_quality_score = sum(
                ...
                if keyword.lower() in real_prompt.lower()
            )
            ...
            print("❌ Prompt generator not found or not initialized")
            return False
...
if __name__ == "__main__":
    success = asyncio.run(test_real_prompt_generator())
