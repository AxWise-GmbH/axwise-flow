...
import json
import logging
from typing import Any, Dict, List, Union, Optional, Type, TypeVar, Generic, get_type_hints
...
class PydanticParser:
    ...
    def parse_json(json_str: str, model: Type[T], context: str = "") -> Optional[T]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Pydantic validation error after repair in context {context}: {str(e)}")
                # Try partial parsing
                return PydanticParser._partial_parse(repaired, model, context)
            except json.JSONDecodeError as e:
                logger.error(f"JSON decode error after repair in context {context}: {str(e)}")
                return None
        except Exception as e:
    ...
    def parse_list(json_str: str, model: Type[T], context: str = "") -> List[T]:
        ...
        try:
            ...
            if not isinstance(data, list):
                logger.warning(f"Expected list but got {type(data).__name__} in context {context}")
                if isinstance(data, dict):
                    # Check if there's a list field in the dict
                    for key, value in data.items():
                        if isinstance(value, list):
                            logger.info(f"Found list in field '{key}' in context {context}")
                            data = value
                            break
                    else:
            ...
            for item in data:
                try:
                    ...
                    logger.warning(f"Validation error for list item in context {context}: {str(e)}")
                    # Try partial parsing for this item
                    parsed_item = PydanticParser._partial_parse_dict(item, model, context)
                    if parsed_item:
            ...
            logger.error(f"Unexpected error parsing JSON list in context {context}: {str(e)}")
            # Continue to repair and retry
        ...
        try:
            ...
            try:
                ...
                if not isinstance(data, list):
                    logger.warning(f"Expected list but got {type(data).__name__} after repair in context {context}")
                    if isinstance(data, dict):
                        # Check if there's a list field in the dict
                        for key, value in data.items():
                            if isinstance(value, list):
                                logger.info(f"Found list in field '{key}' after repair in context {context}")
                                data = value
                                break
                        else:
                            # If no list field found, wrap the dict in a list
                ...
                for item in data:
                    try:
                        ...
                        logger.warning(f"Validation error for list item after repair in context {context}: {str(e)}")
                        # Try partial parsing for this item
                        parsed_item = PydanticParser._partial_parse_dict(item, model, context)
                        if parsed_item:
                            result.append(parsed_item)
    ...
    @staticmethod
    def _partial_parse_dict(data: Dict[str, Any], model: Type[T], context: str) -> Optional[T]:
        """
        ...
        try:
            ...
            logger.error(f"Error during partial dict parsing in context {context}: {str(e)}")
            return None
