...
based on the content and question patterns.
"""
...
class PersonaRouter:
    """Routes interview data to the appropriate persona analyzer"""
    
    # Business workflow question patterns
    BUSINESS_PATTERNS = [
        'tool', 'use', 'process', 'template', 'checklist',
        'method', 'approach', 'customize', 'collaborate', 'share',
        'review', 'standardization', 'guidelines', 'responsibility',
        'task', 'ensure', 'freedom', 'personalize', 'adapt',
        'balance', 'align', 'approve', 'design', 'create', 'modify'
    ]
    
    # Customer experience question patterns
    CUSTOMER_PATTERNS = [
        'experience', 'customer', 'user', 'client', 'buyer',
        'purchase', 'shop', 'find', 'look for', 'search',
        'prefer', 'like', 'want', 'need', 'appealing',
        'frustrat', 'annoying', 'journey', 'touchpoint',
        'authentic', 'local', 'unique', 'special'
    ]
            ...
            if 'respondents' in segment:
                for respondent in segment['respondents']:
                    for answer in respondent.get('answers', []):
                        ...
                        business_matches = sum(1 for pattern in cls.BUSINESS_PATTERNS 
                                             if pattern in question)
                        business_score += business_matches
                        
                        # Count customer patterns
                        customer_matches = sum(1 for pattern in cls.CUSTOMER_PATTERNS 
                                             if pattern in question)
        ...
        for data_segment in data:
            try:
                ...
                persona['routing_metadata'] = {
                    ...
                    'analyzer_used': 'customer' if analysis_type == 'customer' else 'business'
                }
                
                personas.append(persona)
                
            except Exception as e:
                print(f"Error processing persona {data_segment.get('persona_type', 'Unknown')}: {str(e)}")
                continue
        ...
        for data_segment in data:
            try:
                ...
                try:
                    ...
                    customer_persona = customer_analyzer.generate_customer_persona_profile()
                except Exception as e:
                ...
                if business_persona and customer_persona:
                    ...
                    hybrid_persona = cls._merge_personas(business_persona, customer_persona)
                    ...
                    print(f"Both analyzers failed for {data_segment.get('persona_type', 'Unknown')}")
                
            except Exception as e:
                print(f"Error in hybrid analysis for {data_segment.get('persona_type', 'Unknown')}: {str(e)}")
                continue
        
        return personas
    
    @classmethod
    def _merge_personas(cls, business_persona: Dict[str, Any], customer_persona: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        hybrid = {
            ...
            'collaboration_patterns': business_persona.get('collaboration_patterns', {}),
...
def create_smart_personas_from_interviews(interview_file: str, use_hybrid: bool = False) -> List[Dict[str, Any]]:
    """
