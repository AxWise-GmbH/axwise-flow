...
# Import the improved PydanticAI-based functions from nlp_processor
...
class CustomerPersonaAnalyzer:
    ...
    def __init__(
        self, respondents: List[Dict[str, Any]], persona_type: str = "Customer"
    ):
        """
    ...
    def analyze_customer_pain_points(self) -> Dict[str, Any]:
        ...
        if not frustration_responses and not problem_responses:
            return {
                ...
                "key_problems": [],
                "improvement_desires": [],
                "pain_sentiment": {"average_polarity": 0, "average_subjectivity": 0},
            }
        ...
        if total_sentiments > 0:
            avg_polarity = (
                sum(s["polarity"] for s in pain_sentiments) / total_sentiments
            )
            avg_subjectivity = (
                sum(s["subjectivity"] for s in pain_sentiments) / total_sentiments
            )
        else:
            avg_polarity = 0
            avg_subjectivity = 0
    ...
    def analyze_customer_journey(self) -> Dict[str, Any]:
        ...
        return {"journey_patterns": {}, "representative_quotes": {}}
    ...
    def generate_customer_persona_profile(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"Generated customer profile with {len(profile['customer_attributes'].get('behaviors', []))} behaviors, "
            ...
            print(
                f"Error generating customer profile for {self.persona_type}: {str(e)}"
            )
    ...
    def _extract_representative_quotes(self) -> Dict[str, List[str]]:
        ...
        for respondent in self.respondents:
            for answer in respondent["answers"]:
                ...
                if any(
                    pattern in lower_question
                    for pattern in ["prefer", "like", "want", "appealing", "ideal"]
                ):
                ...
                if any(
                    pattern in lower_question
                    for pattern in ["experience", "approach", "usually", "how do you"]
                ):
...
def create_customer_personas_from_interviews(
    interview_file: str,
) -> List[Dict[str, Any]]:
    ...
    for data_segment in interview_data:
        analyzer = CustomerPersonaAnalyzer(
            data_segment["respondents"], data_segment.get("persona_type", "Customer")
        )
